...
# seed_list=($(seq 1 1 30))
# seed_list=($(seq 1 1 10))
        ...
        if [ ${mode} = 'train' ]; then
            ...
            for trial_nbr in "${trial_list[@]}"; do
                ...
                for seed_nbr in "${seed_list[@]}"; do
                    ...
                    if [ ${need_gpu} = 1 ]; then
                        ...
                        $CBIG_CODE_DIR/setup/CBIG_pbsubmit -cmd "$cmd" -walltime 18:00:00 -ncpus 1 -ngpus 1 -mem 2G -name "se${seed_nbr}t${trial_nbr}_train" -joberr "$logdir/$logerror" -jobout "$logdir/$log_out"
                    elif [ ${need_gpu} = 0 ]; then
                        cmd="source activate ${conda_env}; cd ${proj_dir}; python -u ${main_py} $trial_nbr $seed_nbr"
                        $CBIG_CODE_DIR/setup/CBIG_pbsubmit -cmd "$cmd" -walltime 45:00:00 -ncpus 1 -mem 2G -name "se${seed_nbr}t${trial_nbr}_train" -joberr "$logdir/$logerror" -jobout "$logdir/$log_out"
                    fi
                done
            done
            ...
            for trial_nbr in "${trial_list[@]}"; do
                ...
                for seed_nbr in "${seed_list[@]}"; do
                    ...
                    if [ ${need_gpu} = 1 ]; then
                        cmd="module load cuda/11.7; source activate ${conda_env}; cd ${proj_dir}; python -u ${main_py} $trial_nbr $seed_nbr"
                        $CBIG_CODE_DIR/setup/CBIG_pbsubmit -cmd "$cmd" -walltime 6:00:00 -ncpus 1 -ngpus 1 -mem 2G -name "se${seed_nbr}t${trial_nbr}_val" -joberr "$logdir/$logerror" -jobout "$logdir/$log_out"
                    elif [ ${need_gpu} = 0 ]; then
                        cmd="source activate ${conda_env}; cd ${proj_dir}; python -u ${main_py} $trial_nbr $seed_nbr"
                        $CBIG_CODE_DIR/setup/CBIG_pbsubmit -cmd "$cmd" -walltime 25:00:00 -ncpus 1 -mem 2G -name "se${seed_nbr}t${trial_nbr}_val" -joberr "$logdir/$logerror" -jobout "$logdir/$log_out"
                    fi
                done
            done
