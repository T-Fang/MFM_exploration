...
from src.utils.file_utils import get_agg_seeds_range, get_prev_phase_best_params_path, get_curr_phase_save_dir, get_best_train_params, get_HCPYA_group_stats, get_HCPYA_group_stats_for_phase, get_best_params_file_path, get_sim_res_dir, get_run_dir, get_target_dir  # noqa
...
def extract_phase(mode):
    if mode == 'train' or mode == 'val' or mode == 'test':
        return mode
    elif mode == 'best_from_train':
        return 'train'
    elif mode.startswith('best_from_prev_phase_sim_on'):
        return mode.split('_')[-1]
    else:
        raise ValueError("Invalid mode.")


def apply_on_all_participants(mode, trial_idx, seed_idx):
    DS_NAME = 'HCPYA'
    set_torch_default()
    ...
    AGG_SEEDS_NUM = None  # ! the number of seeds to aggregate, set to None if we do not want to aggregate seeds during validation and test
    ...
    curr_phase_save_dir = get_curr_phase_save_dir(ds_name=DS_NAME,
                                                  target=TARGET,
                                                  phase=phase,
                                                  trial_idx=trial_idx,
                                                  seed_idx=seed_idx,
    ...
    # ! Other than training, we should consider the possibility of aggregated seeds
    if AGG_SEEDS_NUM is not None:
        seed_range, seed_idx = get_agg_seeds_range(AGG_SEEDS_NUM, seed_idx)
    else:
        seed_range = [seed_idx]
    ...
    if mode == 'best_from_train':
        ...
        for seed_idx in seed_range:
            train_save_dir = get_run_dir(DS_NAME, TARGET, phase, trial_idx,
                                         seed_idx)
...
def apply_large_group(mode, trial_nbr, seed_nbr, epoch=None):
    ...
    if mode == 'train':
        emp_stats = get_HCPYA_group_stats(860, 917)
        ...
        emp_stats = get_HCPYA_group_stats(917, 973)
        ...
        if epoch is None:
            for ep in range(0, epochs):
                mfm_validator.val_best_parameters(emp_stats['sc_euler'],
                                                  emp_stats['emp_fc'],
                                                  emp_stats['emp_fcd_cum'], ep)
        else:
            epoch = int(epoch)
            mfm_validator.val_best_parameters(emp_stats['sc_euler'],
                                              emp_stats['emp_fc'],
                                              emp_stats['emp_fcd_cum'], epoch)

    elif mode == 'test':
        emp_stats = get_HCPYA_group_stats(973, 1029)
        ...
        mfm_tester.test(emp_stats['sc_euler'], emp_stats['emp_fc'],
                        emp_stats['emp_fcd_cum'])
...
def apply_individual(mode, sub_nbr, trial_nbr, seed_nbr, epoch=None):
    ...
    if mode == 'train':
        ...
        emp_stats = get_HCPYA_group_stats(
            0, 1)  # TODO: Modify if we want to use this
        state = train_help_function(config=config,
                                    emp_stats=emp_stats,
                                    train_save_dir=save_param_dir,
                                    num_epochs=epochs,
    ...
    #     mfm_validator.val_by_euler(myelin, rsfc_gradient, sc_euler, emp_fc, emp_fcd_cum, epoch)
        ...
        mfm_tester = CMAESTester(config,
                                 ...
                                 test_dir,
                                 train_num_epochs=epochs)
        mfm_tester.test(sc_euler, emp_fc, emp_fcd_cum)
...
def generate_dl_dataset_group(mode, group_nbr, trial_nbr, seed_nbr):
    ...
    emp_stats = get_HCPYA_group_stats_for_phase(
        'train')  # TODO: Modify if need to regenerate the DL dataset
    state = train_help_function(config=config,
                                emp_stats=emp_stats,
                                train_save_dir=save_param_dir,
                                num_epochs=epochs,
...
if __name__ == "__main__":
    ...
    for phase in phases:
        if USE_MULTIPROC:
            ...
            apply_on_all_participants(mode=phase,
