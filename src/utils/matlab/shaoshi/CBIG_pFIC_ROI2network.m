...
% vertices to yeo 7 networks. This function is mainly used to generate the
...
network_def = load('/home/ftian/storage/projects/MFM_exploration/src/utils/matlab/labels/yeo7network_51_roi_label.mat');
...
rh_label_network_fs5(rh_label_network_fs5>0) = rh_label_network_fs5(rh_label_network_fs5>0) + 26; % add an offset to differentiate rh from lh labels
...
for i = 1:7
    ...
    for j = 1:length(component_index)
        ...
        network_index = [network_index; i*ones(length(lh_rh_param_vertex_component{component_index(j)}), 1)]; % from 1 to 7
    end
end
