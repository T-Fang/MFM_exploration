function CBIG_pFIC_draw_surface_maps(input_vertex, fig_title)
...
% Example:
% CBIG_pFIC_draw_surface_maps(vertex_level_EI_ratio_in_fsaveage5, 'EI_ratio')
%
% Written by Shaoshi Zhang, Xiaoxuan Yan and CBIG under MIT license: https://github.com/ThomasYeoLab/CBIG/blob/master/LICENSE.md
...
if(~exist(fullfile(tmp_dir, [fig_title '.mat']),'file'))
    ...
    load(fullfile(tmp_dir, [fig_title '.mat']));
end
...
% difference after FDR. Thus, the temporal pole is masked out when plotting the surface map and it is shown in silver grey)
if sum(isinf(unique_vertex)) ~= 0
    ct_large(end, :) = [192 192 192]; 
end

load('/home/ftian/storage/projects/MFM_exploration/src/utils/matlab/labels/yeo7network_fsLR164k_label.mat', 'lh_yeo_label_fsLR_164k', 'rh_yeo_label_fsLR_164k');
...
[yeo_lh_boundary, yeo_rh_boundary] = CBIG_pFIC_BuildTwoVertThickBoundary(lh_avg_mesh_fs164k, rh_avg_mesh_fs164k, lh_yeo_label_fsLR_164k, rh_yeo_label_fsLR_164k);
...
command = ['freeview -f $CBIG_CODE_DIR/data/templates/surface/fs_LR_164k/surf/lh.very_inflated:annot=' ...
    lh_annot ':color=5,25,5:edgethickness=0 --viewport 3d -cam dolly 1.6 -ss ' fig_title '_lh_lateral.png'];
system(command);
command = ['freeview -f $CBIG_CODE_DIR/data/templates/surface/fs_LR_164k/surf/lh.very_inflated:annot=' ...
    lh_annot ':color=5,25,5:edgethickness=0 --viewport 3d -cam dolly 1.6 Azimuth 180 -ss ' fig_title '_lh_medial.png'];
system(command);
command = ['freeview -f $CBIG_CODE_DIR/data/templates/surface/fs_LR_164k/surf/rh.very_inflated:annot=' ...
    rh_annot ':color=5,25,5:edgethickness=0 --viewport 3d -cam dolly 1.6 -ss ' fig_title '_rh_medial.png'];
system(command);
command = ['freeview -f $CBIG_CODE_DIR/data/templates/surface/fs_LR_164k/surf/rh.very_inflated:annot=' ...
    rh_annot ':color=5,25,5:edgethickness=0 --viewport 3d -cam dolly 1.6 Azimuth 180 -ss ' fig_title '_rh_lateral.png'];
system(command);

%% trim the white bezel of the output images
cmd = CBIG_TrimFreeviewScreenshotCommand([fig_title '_lh_lateral.png'], [fig_title '_lh_lateral.png']);
system(cmd);
cmd = CBIG_TrimFreeviewScreenshotCommand([fig_title '_lh_medial.png'], [fig_title '_lh_medial.png']);
system(cmd);
cmd = CBIG_TrimFreeviewScreenshotCommand([fig_title '_rh_lateral.png'], [fig_title '_rh_lateral.png']);
system(cmd);
cmd = CBIG_TrimFreeviewScreenshotCommand([fig_title '_rh_medial.png'], [fig_title '_rh_medial.png']);
system(cmd);
