...
def compare_ica_acompcor():
    ...
    for group_nbr in range(1, 30):
        ...
        fc_fcd_acomp = sio.loadmat(os.path.join(group_mats_path, 'train', 'fc_fcd_cdf_aCompCor.mat'))
        fc_fcd_ica = sio.loadmat(os.path.join(group_mats_path, 'train', 'fc_fcd_cdf.mat'))
    ...
    fc_dataframe.to_csv('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_mean_ica_acomp.txt', sep='\t', index=False)
    print("Saved.")
...
def roi_fc_to_network_fc():
    ...
    sublist = pd.read_csv('/mnt/nas/CSC7/Yeolab/Data/HCP/HCP_Aging/postprocessing/HCA_sublist_all.txt', header=None, index_col=False)
...
def check_within_between_fc():
    ...
    id_age_file = pd.read_csv('/mnt/nas/CSC7/Yeolab/Data/HCP/HCP_Aging/postprocessing/HCA_demographics.txt', sep='\t', header=0, index_col=False)
    save_fig_dir = '/home/tzeng/storage/Python/MFMApplication/figures/HCA'
...
def roi_fc_vs_age():
    ...
    id_age_df = pd.read_csv('/mnt/nas/CSC7/Yeolab/Data/HCP/HCP_Aging/postprocessing/HCA_demographics.txt', sep='\t', header=0, index_col=False)
    roi_num = 400
    ...
    fc_mean, fc_develop = usage_functions.roi_fc_vs_age(roi_fc_dir, id_age_df, roi_num)

    sio.savemat(os.path.join(save_dir, f'fc_mean{save_postfix}.mat'), {'fc': fc_mean})
    sio.savemat(os.path.join(save_dir, f'fc_develop{save_postfix}.mat'), {'fc': fc_develop})
    print("Saved.")


def param_distribution():
    test_dir = '/home/tzeng/storage/Python/MFMApplication/Params/HCAParams/group/test/trial5'
    x_list = []
    wee_list = []
    wei_list = []
    for group_nbr in range(1, 30):
    ...
    plt.ylabel('param')
    plt.savefig('/home/tzeng/storage/Python/MFMApplication/usage_files/figures2/HCA_trial5_wee.png')
    plt.close()
    ...
    plt.xlabel('group nbr')
    plt.ylabel('param')
    plt.savefig('/home/tzeng/storage/Python/MFMApplication/usage_files/figures2/HCA_trial5_wei.png')
    plt.close()


def param_vs_age():
    trial_nbr = 3
    test_dir = f'/home/tzeng/storage/Python/MFMApplication/Params/HCAParams/group/test/trial{trial_nbr}'
    age_list = []
    wee_list = []
    wei_list = []
    for group_nbr in range(1, 30):
