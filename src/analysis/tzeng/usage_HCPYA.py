...
def final_state_check():
    train_dir = f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/DL_dataset/Yan100/train/trial1/seed1'
    count = 0
    for nbr in range(0, 64):
        train_final_state = os.path.join(train_dir, f'group{nbr}/final_state.pth')
...
def roi_fc_to_network_fc():
    ...
    sublist = sio.loadmat('/home/tzeng/storage/Matlab/HCPS1200/matfiles/all_mats_1029/subject_1029.mat')
...
def check_within_between_fc():
    ...
    id_age_file = pd.read_csv('/home/tzeng/storage/Matlab/HCPS1200/txt_files/HCPYA_ages_1029.txt', sep='\t', header=0, index_col=False)
    save_fig_dir = '/home/tzeng/storage/Python/MFMApplication/figures/HCPYA'
...
def remove_empty_dir():
    test_dir = '/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/individual/test'
...
def compare_group_individual():
    ...
    for sub_nbr in range(684, 901):
        group_path = f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/test_groupmats/trial1/sub{sub_nbr}/test_results.pth'
        if not os.path.exists(group_path):
            continue
        b = torch.load(group_path)
        if len(b['corr_loss']) > 0:
            a = torch.load(f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/individual/test/sub{sub_nbr}/test_results.pth')
            indi_list.append(torch.min(a['corr_loss'] + a['l1_loss'] + a['ks_loss']).numpy())
            group_list.append((b['corr_loss'] + b['l1_loss'] + b['ks_loss']).numpy()[0])
    df = pd.DataFrame([])
    df['group'] = group_list
    df['individual'] = indi_list
    df.to_csv('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/files/compare.txt', sep='\t', header=True, index=False)
    ...
    tzeng_func.tzeng_2_sample_t_test_n_plot(group_list, indi_list, need_boxplot=True, labels=['group', 'individual'], save_fig_path='/home/tzeng/storage/Python/MFMApplication/usage_files/figures2/HCPYA/compare_group_indi.png', fig_title='Compare Group Individual', xlabel='', ylabel='Loss')


def visualize_FCD():
    epoch = 99
    fcd_mat = torch.load(f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/sim_fc_fcd/fcd/val_epoch{epoch}.pth')
    fcd_mat = fcd_mat['fcd'].numpy()
    for i in range(3):
        tmp_fcd = fcd_mat[i]
        save_fig_path = f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/sim_fc_fcd/visualize_fcd/e{epoch}_{i}.png'
        tzeng_func.tzeng_visualize_FCD(tmp_fcd, save_fig_path)


def check_test_results_params():
    parent_path = '/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/test/trial9/split1'
    count = 0
    rec = np.zeros((106, 10))
    for sub_nbr in range(860, 1029):
        sub_path = os.path.join(parent_path, f'sub{sub_nbr}', 'test_results.pth')
...
def read_train_logs(trial_nbr):
    ...
    record_file.to_csv(f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial{trial_nbr}_randseed_.txt', sep=' ', header=False, index=False)
    return 0
...
def combine_train_logs(origin_txt_path, new_txt_path, save_path):
    ...
    origin_txt = pd.read_csv(origin_txt_path, sep=' ', header=None, index_col=False)
        ...
        for j in range(len(new_txt)):
            if new_txt.iloc[j, 0] == origin_txt.iloc[i, 0]:
                if new_txt.iloc[j, 1] < 0:
                    origin_txt.iloc[i, 1] = origin_txt.iloc[i, 1] + new_txt.iloc[j, 1]
                else:
                    origin_txt.iloc[i, 1] = -origin_txt.iloc[i, 1] + new_txt.iloc[j, 1]
                break
...
def stat_random_seed(trial_nbr_list):
    trial_9 = pd.read_csv('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial9_randseed.txt', sep=' ', header=None, index_col=False)
    randseed_array = np.ones((len(trial_9),))
    for trial_nbr in trial_nbr_list:
        trial_randseed_file = f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial{trial_nbr}_randseed.txt'
        trial_randseed_file = pd.read_csv(trial_randseed_file, sep=' ', header=None, index_col=False)
...
def check_logfile_validation(trial_nbr):
    for sub_nbr in range(860, 1029):
        ...
        empty_list, _, nonexist_list = tzeng_func.tzeng_check_logfile_empty(log_dir, loglist, logfile_prefix='e', logfile_postfix=f't{trial_nbr}_out.log')
        if len(empty_list) > 0:
...
def check_logfile_test(trial_nbr):
    ...
    empty_list, _, nonexist_list = tzeng_func.tzeng_check_logfile_empty(log_dir, loglist, logfile_prefix='s', logfile_postfix=f't{trial_nbr}_out.log')
    if len(empty_list) > 0:
        print(empty_list)
...
def select_best_from_validation():
    ...
    val_dirs = ['/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/val/trial1']
    test_dir = '/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/val/best_from_trial1'
...
def compare_train_logs_main(trial_nbr):
    combine_train_logs(f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial{trial_nbr}_randseed.txt', f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial{trial_nbr}_randseed_.txt', f'/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/individual/record_files/trial{trial_nbr}_randseed.txt')
...
def group_sim_fc_fcd_to_individual(sub_nbr, group_same_fc_fcd):
    ...
    total_loss, corr_loss, L1_loss, ks_loss = MfmModel2014.all_loss_calculate_from_fc_fcd(fc_sim, fcd_pdf, fc_emp, emp_fcd_cum)
    save_loss = torch.hstack((corr_loss.unsqueeze(1), L1_loss.unsqueeze(1), ks_loss.unsqueeze(1)))

    return save_loss


def group_sim_fc_fcd_to_individual_main():
    group_same_fc_fcd = torch.load('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/group_340/simulate/trial3/seed1/sim_results.pth')
    save_losses = torch.zeros((109, 10, 3))
    sub_109 = [862, 863, 864, 865, 866, 868, 869, 870, 871, 872, 873, 874, 875, 877, 879, 881, 883, 885, 888, 889, 890, 891, 892, 895, 896, 897, 900, 901, 902, 904, 905, 907, 911, 912, 914, 916, 918, 919, 921, 924, 925, 929, 930, 931, 933, 935, 936, 937, 938, 939, 941, 943, 944, 945, 946, 949, 950, 951, 952, 953, 954, 955, 956, 960, 962, 964, 966, 967, 968, 969, 970, 971, 972, 973, 974, 976, 977, 979, 981, 983, 984, 986, 987, 988, 989, 990, 991, 993, 994, 996, 998, 999, 1001, 1002, 1003, 1004, 1006, 1008, 1014, 1016, 1017, 1019, 1021, 1022, 1023, 1024, 1025, 1026, 1028]
    for s_nbr in range(len(sub_109)):
        save_losses[s_nbr] = group_sim_fc_fcd_to_individual(sub_109[s_nbr], group_same_fc_fcd)
    torch.save(save_losses, '/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/group_340/simulate/trial3/seed1/loss_group_to_subj_2RLtest_MAE.pth')


def compute_KS_distance():
    fcd_mat = torch.load('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/compare_group_individual/group/sim_fc_fcd/fcd/val_epoch99.pth')
    fcd_mat = fcd_mat['fcd']
    window_num = fcd_mat.shape[1]
    bins = 10000
    fcd_mask = torch.triu(torch.ones(window_num, window_num, dtype=torch.bool), 1)
...
def visualize_FC():
    ...
    fc = torch.load('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/group_340/simulate/trial1/sim_results_5seeds.pth')
    fc = fc['fc'][0].numpy()
    plt.imshow(fc)
    plt.savefig('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/group_340/figures/visualize_fc_fcd/fc_sim_trial1_5seeds.png')
    plt.close()
...
def simulate_results_loss_analysis():
    ...
    fc_emp = tzeng_func.tzeng_fisher_average(fc_emp)
    fc_emp = torch.as_tensor(fc_emp)
    fcd_1029 = sio.loadmat('/home/tzeng/storage/Matlab/HCPS1200/matfiles/all_mats_1029/fcd_cum_1029.mat')
    fcd_1029 = fcd_1029['fcd_cum_1029']
    emp_fcd_cum = torch.as_tensor(fcd_1029[subrange[0]:subrange[1]].astype(np.float64))
    emp_fcd_cum = torch.mean(emp_fcd_cum, dim=0)
    emp_fcd_cum = (emp_fcd_cum / emp_fcd_cum[-1]).unsqueeze(1)

    sim_res = torch.load('/home/tzeng/storage/Python/MFMApplication/Params/HCPYAParams/group_340/simulate/trial3/seed1/sim_results_mats.pth')
    print(sim_res['fcd_pdf'].shape)
    fc_sim = sim_res['fc']
    fc_losses = np.zeros((10, 3))
    for i in range(10):
        _, fc_losses[i] = MfmModel2014.FC_correlation_n_L1_cost(fc_sim[i], fc_emp)
    print(fc_losses)
