...
def plot_train_val_loss(loss_file_dir, loss_file_path):
    loss_file = pd.read_csv(os.path.join(loss_file_dir, loss_file_path + '.csv'), sep=',', header=0, index_col=False)
...
def plot_pred_loss(epochs, param_save_dir, figure_path):
    ...
    for i in range(epochs):
        d = torch.load(os.path.join(param_save_dir, f'param_save_epoch{i}.pth'))
        if 'corr_loss' in d:
...
def check_dl_version_test_results_one_epoch(train_results_dir, val_results_dir, epoch, save_compare_figure=False, save_figure_dir=None):
    count = 0
    mse_corr_total = 0
    mse_l1_total = 0
    mse_ks_total = 0
    ...
    d_dl = torch.load(os.path.join(train_results_dir, f'param_save_epoch{epoch}.pth'))
    
    test_results_path = os.path.join(val_results_dir, f'Euler_epoch{epoch}.pth')
    ...
    if os.path.exists(test_results_path):
        ...
        res = (mean_mse_corr, mean_mse_l1, mean_mse_ks, dl_valid_param_length, valid_param_length, count,
                np.mean(corr_dl_list), np.mean(corr_euler_list),
                np.mean(l1_dl_list), np.mean(l1_euler_list),
                np.mean(ks_dl_list), np.mean(ks_euler_list))
        ...
        res = (0, 0, 0, valid_param_length, valid_param_length, valid_param_length, 
                corr_, corr_, l1_, l1_, ks_, ks_)
    # End if
    loss_name_list = ['corr', 'l1', 'ks']
    if save_compare_figure:
        if save_figure_dir is None:
            raise Exception("You need to specify your figure saving directory.")
        ...
        for i in range(3):
            ...
            plt.title(title)
            plt.legend()
            plt.savefig(os.path.join(save_figure_dir, f'Compare_epoch{epoch}_{loss_name}' + '.png'))
            plt.close()
...
def check_dl_version_test_results_epochs(train_results_dir, val_results_dir, save_dir, epochs, distinct_name, need_plot_overlay_dl_euler=True, need_save_record=True, need_plot_ground_euler=False, ground_dir=None):

    record_list = np.zeros((epochs, 6))
    record_dict = {'mse_corr': np.zeros((epochs, ), dtype=np.float64), 
    ...
    for i in range(epochs):
        res = check_dl_version_test_results_one_epoch(train_results_dir, val_results_dir, i)
        ...
        record_list[i] = [corr_dl, corr_euler, l1_dl, l1_euler, ks_dl, ks_euler]
    ...
    if need_plot_overlay_dl_euler:
        ...
        plt.figure()
        plt.plot(x, record_list[:, 0], color='#ff0000', ls='-', label='Pred corr loss')
        plt.plot(x, record_list[:, 2], color='#0000ff', ls='-', label='Pred L1 loss')
        plt.plot(x, record_list[:, 4], color='#00ff00', ls='-', label='Pred KS loss')
        plt.plot(x, record_list[:, 1], color='#ff6347', ls=':', label='Euler validated corr')
        plt.plot(x, record_list[:, 3], color='#4169e1', ls=':', label='Euler validated L1')
        plt.plot(x, record_list[:, 5], color='#32cd32', ls=':', label='Euler validated KS')
        plt.xlabel('epoch')
        plt.ylabel('loss')
        plt.title(f'Pred and Euler validated')
        plt.legend()
        plt.savefig(os.path.join(tmp_dir, f'Pred_and_Euler_{distinct_name}.png'))
        plt.close()
    ...
    if need_save_record:
        ...
        dataframe.to_csv(os.path.join(tmp_dir, f'Compare_DL_Euler_{distinct_name}.csv'), sep=',')
    ...
    if need_plot_ground_euler:
        ...
        for i in range(epochs):
            d_ground = torch.load(os.path.join(ground_dir, f'param_save_epoch{i}.pth'))
        ...
        plt.figure()
        plt.plot(x, ground_list[:, 0], color='#ff0000', ls='-', label="Shaoshi's approach corr")
        plt.plot(x, ground_list[:, 1], color='#0000ff', ls='-', label="Shaoshi's approach L1")
        plt.plot(x, ground_list[:, 2], color='#00ff00', ls='-', label="Shaoshi's approach KS")
        plt.plot(x, record_list[:, 1], color='#ff6347', ls=':', label='hybrid corr validated by Euler')
        plt.plot(x, record_list[:, 3], color='#4169e1', ls=':', label='hybrid L1 validated by Euler')
        plt.plot(x, record_list[:, 5], color='#32cd32', ls=':', label='hybrid KS validated by Euler')
        plt.xlabel('epoch')
        plt.ylabel('loss')
        plt.title(f'Ground and hybrid losses')
        plt.legend()
        plt.savefig(os.path.join(tmp_dir, f'Ground_and_Euler_{distinct_name}.png'))
        plt.close()
        print(f"Figure saved to {tmp_dir}.")


def compare_test_results_hybrid_normal(hybrid_test_res_path, normal_test_res_path, need_boxplot, figure_dir=None, figure_title=None):
    if need_boxplot:
        if figure_dir is None or figure_title is None:
            raise Exception("You need to specify the figure saving directory and figure title.")
    ...
    print('mean: ', torch.mean(hybrid_total_loss), torch.mean(normal_total_loss))
...
def compare_train_val_test(train_dirs, val_dirs, test_path, need_boxplot, figure_dir=None, figure_name=None):
    ...
    if need_boxplot:
        if figure_dir is None or figure_name is None:
            raise Exception("You must specify your figure save directory and figure name.")
        if not os.path.exists(figure_dir):
    ...
    for i in range(len(trial_nums)):
        ...
        # for train
        train_path = os.path.join(train_dirs[trial_nbr], f'param_save_epoch{epoch}.pth')
        train_res = torch.load(train_path)
        if 'corr_loss' in train_res:
            train_loss_tmp = train_res['corr_loss'] + train_res['L1_loss'] + train_res['ks_loss']
    ...
    if need_boxplot:
        ...
        plt.rcParams['font.size'] = '8'
        all_arr = np.hstack((
            train_corr.unsqueeze(1).numpy(), val_corr.unsqueeze(1).numpy(), test_corr.unsqueeze(1).numpy(),
            train_l1.unsqueeze(1).numpy(), val_l1.unsqueeze(1).numpy(), test_l1.unsqueeze(1).numpy(),
            train_ks.unsqueeze(1).numpy(), val_ks.unsqueeze(1).numpy(), test_ks.unsqueeze(1).numpy(),
            train_total.unsqueeze(1).numpy(), val_total.unsqueeze(1).numpy(), test_total.unsqueeze(1).numpy()
        ))
        plt.boxplot(all_arr,
                    labels=[
                        'Train\nco_loss', 'Val\nco_loss', 'Test\nco_loss', 
...
def plot_EI_ratio(EI_list, age_list, save_fig_path, xlabel='age', ylabel='mean cortical E/I ratio'):
    EI_list = np.array(EI_list)
    age_list = np.array(age_list) / 12
    corr = np.corrcoef(EI_list.reshape(1, -1), age_list.reshape(1, -1))[0, 1]
    plt.figure()
    sns.regplot(x=age_list, y=EI_list, scatter_kws={'color': '#696969'}, line_kws={'color': 'red'}, order=1)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
...
def compare_test_results_two_lists(test_dir_1, test_dir_2, best_val_or_test_mean, prefix, nbr_range):
    ...
    # for sub_nbr in sub_list:
        ...
        if best_val_or_test_mean == 'best_val':
            loss_lists[0, valid_count, 1:] = [res_1['corr_loss'][0], res_1['l1_loss'][0], res_1['ks_loss'][0]]
            loss_lists[1, valid_count, 1:] = [res_2['corr_loss'][0], res_2['l1_loss'][0], res_2['ks_loss'][0]]
        elif best_val_or_test_mean == 'test_mean':
            loss_lists[0, valid_count, 1:] = [torch.mean(res_1['corr_loss']), torch.mean(res_1['l1_loss']), torch.mean(res_1['ks_loss'])]
            loss_lists[1, valid_count, 1:] = [torch.mean(res_2['corr_loss']), torch.mean(res_2['l1_loss']), torch.mean(res_2['ks_loss'])]
        else:
            raise Exception("best val or test mean.")
        valid_count += 1
    print("Valid count: ", valid_count)
    loss_lists = loss_lists[:, :valid_count]
    loss_lists[:, :, 0] = loss_lists[:, :, 1] + loss_lists[:, :, 2] + loss_lists[:, :, 3]

    return loss_lists


def get_test_results_from_many_dirs(test_dirs, best_val_or_test_mean, prefix, nbr_range):
    ...
    for nbr in nbr_range:
        ...
        for i_dir in range(n_dir):
            cur_path = os.path.join(test_dirs[i_dir], f'{prefix}{nbr}/test_results.pth')
        ...
        for i_dir in range(n_dir):
            ...
            if best_val_or_test_mean == 'best_val':
                loss_lists[i_dir, valid_count, 1:] = [cur_res['corr_loss'][0], cur_res['l1_loss'][0], cur_res['ks_loss'][0]]
            elif best_val_or_test_mean == 'test_mean':
                loss_lists[i_dir, valid_count, 1:] = [torch.mean(cur_res['corr_loss']), torch.mean(cur_res['l1_loss']), torch.mean(cur_res['ks_loss'])]
            else:
                raise Exception("best val or test mean.")
        valid_count += 1
    loss_lists = loss_lists[:, :valid_count]
...
def test_main():
    test_dir_1 = '/home/tzeng/storage/Python/MFMApplication/Params/PNCParams/group_by_45/test/trial5'
    test_dir_2 = '/home/tzeng/storage/Python/MFMApplication/Params/PNCParams/group_by_45/test/trial6'
    best_val_or_test_mean = 'best_val'
    prefix = 'group'
    nbr_range = np.arange(1, 2)
    compare_test_results_two_lists(test_dir_1, test_dir_2, best_val_or_test_mean, prefix, nbr_range)
