...
def EI_ratio_group_parameter(group_nbr, trial_nbr):
    ...
    param_path = os.path.join(parent_dir, f'test/trial{trial_nbr}/group{group_nbr}/val_results.pth')
    ...
    EI_save_path = os.path.join(parent_dir, f'EI_ratio/trial{trial_nbr}/group{group_nbr}.pth')
...
def map_desikan_to_yeo7():
    ...
    print(mapping_dict)
    sio.savemat('/home/tzeng/storage/Matlab/Utils/general_mats/yeo7_to_desikan68.mat', mapping_dict)
    print('Saved.')
...
def roi_fc_to_network_fc():
    ...
    network_dlabel = sio.loadmat('/home/tzeng/storage/Matlab/Utils/general_mats/Yeo17_fslr32.mat')
    ...
    sublist = pd.read_csv('/mnt/nas/CSC7/Yeolab/Data/HCP/HCP_Development/postprocessing/HCD_sublist_all.txt', header=None, index_col=False)
    ...
    usage_functions.roi_fc_2_network_fc(network_dlabel, roi_dlabel, sublist, roi_fc_dir, save_roi2network_dir)
...
def roi_fc_to_network_fc_desikan():
    ...
    usage_functions.roi_fc_2_network_fc_desikan(sublist, roi_fc_dir, save_roi2network_dir)
...
def roi_fc_to_network_fc_group_desikan():
    ...
    for group_nbr in range(1, group_num + 1):
        ...
        roi_fc_path = os.path.join(roi_fc_dir, f'{group_nbr}/train/fc_fcd_cdf.mat')
        ...
        sio.savemat(os.path.join(save_reorganize_roi_dir, f'group{group_nbr}.mat'), {'fc': reorganize_fc})
...
def roi_fc_to_network_fc_group_Yan():
    ...
    for group_nbr in range(2, 30):
        sub_demo = pd.read_csv(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/age_group/{group_nbr}/subjects_demo.txt', sep='\t', header=None, index_col=False)
        fc_cat = None
...
def check_within_between_fc():
    ...
    id_age_file = pd.read_csv('/mnt/nas/CSC7/Yeolab/Data/HCP/HCP_Development/postprocessing/HCD_demographics.txt', sep='\t', header=0, index_col=False)
    save_fig_dir = '/home/tzeng/storage/Python/MFMApplication/figures/HCD'
    fig_postfix = '_MNI_wbs_bp_Yan100_Yeo17'

    usage_functions.check_within_between_fc(id_age_file, roi2network_dir, save_fig_dir, fig_postfix)
...
def check_within_between_fc_group_desikan():
    ...
    for group_nbr in range(1, group_num + 1):
        ...
        if not os.path.exists(fc_path):
            continue
        id_age_file = pd.read_csv(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/age_group/{group_nbr}/train/train_subjects_demo.txt', sep='\t', header=None, index_col=False)
    ...
    tzeng_func.tzeng_scatter_with_regress_line(age_list, within_fc_list, os.path.join(save_fig_dir, f'within_fc{fig_postfix}.png'))
    tzeng_func.tzeng_scatter_with_regress_line(age_list, between_fc_list, os.path.join(save_fig_dir, f'between_fc{fig_postfix}.png'))
    tzeng_func.tzeng_scatter_with_regress_line(age_list, whole_fc_list, os.path.join(save_fig_dir, f'whole_fc{fig_postfix}.png'))


def age_match_fc_with_PNC():
    id_age_PNC = pd.read_csv('/mnt/isilon/CSC2/Yeolab/Data/PNC/documentation/rest_subject_age.csv', sep=',', header=None, index_col=False)
    id_age_HCD = pd.read_csv('/home/tzeng/storage/Matlab/HCP_Dev/matfiles/HCD_demo_valid_aftMR.txt', sep='\t', header=0, index_col=False)
    ...
    for i in range(len(count_pnc_list) - 1):
        ...
        for sub_i in range(start_ind, end_ind):
            ...
            fc_sub = pd.read_csv(f'/home/shaoshi.z/storage/MFM/PNC/desikan_FC_FCD_from_surface/FC/sub-{subject_id}.csv', header=None, index_col=False)
    ...
    for i in range(len(count_hcd_list) - 1):
        ...
        for sub_i in range(start_ind, end_ind):
            ...
            fc_sub = sio.loadmat(f'/mnt/isilon/CSC1/Yeolab/Users/tzeng/Matlab/HCP_Dev/matfiles/FC_FCD_rest/{subject_id}_V1_MR.mat')
    ...
    np.save('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_pnc.npy', fc_pnc_list)
    np.save('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_hcd.npy', fc_hcd_list)
    print("Done.")
...
def plot_age_match_fc():
    fc_pnc = np.load('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_pnc.npy')
    fc_hcd = np.load('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_hcd.npy')
    ...
    plt.xlabel('HCD')
    plt.ylabel('PNC')
    plt.savefig('/home/tzeng/storage/Python/MFMApplication/usage_files/files/fc_corr.png')
    plt.close()
...
def plot_reorganize_fc():
    ...
    for i in range(len(network_len_list) - 1):
        for j in range(len(network_len_list) - 1):
            network_fc[i, j] = np.mean(fc_diff[network_len_list[i]:network_len_list[i+1], network_len_list[j]:network_len_list[j+1]])
    # sio.savemat(os.path.join('/home/tzeng/storage/Python/MFMApplication/usage_files/files/network17.mat'), {'fc': network_fc})
    
    d = sio.loadmat('/home/tzeng/storage/CBIG_private/utilities/matlab/figure_utilities/corr_mat_colorscale.mat')
    cmap = d['rbmap2']
    ...
    plt.xlabel('Networks')
    plt.ylabel('Networks')
    plt.savefig('/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/Yan400_Yeo17_reorganize_figures/fc_diff_network.png')
    plt.close()
...
def roi_fc_vs_age():
    ...
    sio.savemat(f"/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/ForPlot/fc_mean{save_postfix}.mat", {'fc': np.mean(fc_all_subjects, axis=0)})
    print("Saved mean.")
    ...
    sio.savemat(f"/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/ForPlot/fc_development{save_postfix}.mat", {'fc': fc_development})
    print("Saved.")
...
def roi2network_fc_vs_age():
    ...
    sio.savemat(f"/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/roi2network/develop_mats/fc_development{save_postfix}.mat", {'fc': fc_development})
    print("Saved.")
...
def compare_pipeline():
    ...
    fc_develop_1 = sio.loadmat(os.path.join(parent_dir, 'develop_mats', 'fslr_ICAFIX.mat'))
    fc_develop_1 = fc_develop_1['fc']
    fc_develop_2 = sio.loadmat(os.path.join(parent_dir, 'develop_mats', 'fslr_aCompCor_bp.mat'))
    ...
    save_fig_path = os.path.join(parent_dir, 'within_plots', f'within_{postfix_1}_{postfix_2}.png')
    usage_functions.plot_equal_line(fc_develop_1[within_mask], fc_develop_2[within_mask], save_fig_path, fig_title='compare within', xlabel=f'{postfix_1}', ylabel=f'{postfix_2}')

    save_fig_path = os.path.join(parent_dir, 'between_plots', f'between_{postfix_1}_{postfix_2}.png')
    usage_functions.plot_equal_line(fc_develop_1[between_mask], fc_develop_2[between_mask], save_fig_path, fig_title='compare between', xlabel=f'{postfix_1}', ylabel=f'{postfix_2}')


def param_vs_age():
    split_nbr = 1
    trial_nbr = 21
    test_dir = f'/home/tzeng/storage/Python/MFMApplication/Params/HCDParams/group/test/trial{trial_nbr}/split{split_nbr}'
    age_list = []
    wee_list = []
    wei_list = []
...
def param_distribution():
    split_nbr = 1
    trial_nbr = 21
    train_dir = f'/home/tzeng/storage/Python/MFMApplication/Params/HCDParams/group/train/trial{trial_nbr}/split{split_nbr}'
    test_dir = f'/home/tzeng/storage/Python/MFMApplication/Params/HCDParams/group/test/trial{trial_nbr}/split{split_nbr}'
    for group_nbr in range(1, 22):
        train_final_state = os.path.join(train_dir, f'group{group_nbr}', 'final_state.pth')
        test_res_path = os.path.join(test_dir, f'group{group_nbr}', 'val_results.pth')
        if not os.path.exists(test_res_path) or not os.path.exists(train_final_state):
            continue
        ...
        record.to_csv(f'/home/tzeng/storage/Python/MFMApplication/usage_files/files/HCD/param_distribution/{group_nbr}.txt', sep='\t', header=False, index=False)
...
def mean_fc_profile_vs_age_group():
    ...
    corr, p_value = CBIG_func.CBIG_corr(fc_list, age_list.astype(np.float64).reshape((-1, 1)), need_pvalue=True)
    df = pd.DataFrame(corr, columns=['corr'])
    df['p_value'] = p_value
    df.to_csv('/home/tzeng/storage/Python/MFMApplication/usage_files/files/HCD/Harvard_mean_fc_profiles_vs_age_group.txt', sep='\t', header=False, index=False)


def mean_fc_profile_vs_age_individual():
    id_age_file = pd.read_csv('/home/tzeng/storage/Matlab/HCP_Dev/txtfiles/HCD_Harvard_demo.txt', sep='\t', header=0, index_col=False)
    ...
    df = pd.DataFrame(corr, columns=['corr'])
    df['p_value'] = p_value
    df.to_csv('/home/tzeng/storage/Python/MFMApplication/usage_files/files/HCD/Harvard_mean_fc_profiles_vs_age_indi.txt', sep='\t', index=False)
...
def site_information():
    ...
    sio.savemat(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/Analysis/site_difference/Yan400_Yeo17_fc_mean_fslr_aCompCor_{site_name}.mat', {'fc': fc_mean})
    sio.savemat(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/pMFM/within_between_FC/Analysis/site_difference/Yan400_Yeo17_fc_dev_fslr_aCompCor_{site_name}.mat', {'fc': fc_development})
...
def sc_fc_corr():
    ...
    for group_nbr in range(1, n_group + 1):
        ...
        fc_mat = torch.load(f'/home/tzeng/storage/Python/MFMApplication/Params/HCDParams/group/simulate/fc/trial7/split1/group{group_nbr}.pth')
...
def sim_fc_emp_fc():
    ...
    n_group = 11
    for group_nbr in range(1, n_group + 1):
        fc_1 = sio.loadmat(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/age_group/{group_nbr}/split1/train/fc_fcd_cdf.mat')
        fc_1 = fc_1['fc']
        fc_1 = fc_1[mask_68]
        fc_2 = sio.loadmat(f'/home/tzeng/storage/Matlab/HCP_Dev/matfiles/age_group/{group_nbr}/split1/train/fc_fcd_cdf_aCompCor.mat')
        fc_2 = fc_2['fc']
        fc_2 = fc_2[mask_68]

        save_fig_path = os.path.join(save_fig_dir, f'{group_nbr}.png')
        corr = CBIG_func.CBIG_corr(fc_1[:, np.newaxis], fc_2[:, np.newaxis])
        usage_functions.plot_equal_line(fc_1, fc_2, save_fig_path, fig_title=f'corr={corr[0, 0]:.4f}', xlabel='ICA FC', ylabel='aCompCor FC')
